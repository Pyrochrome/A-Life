...
proc frame*(a: Anim): int = a.pos + a.offset

proc atPos*(a: Anim, pos: int): bool =
  ## True if we just arrived at a given frame
  a.pos == pos and a.timer == a.speed

proc atEnd*(a: Anim): bool =
...
proc animinit*(a: var Anim, len, speed: int, p: int = 0, offset: int = 0) =
  a.timer = speed + 1
  a.pos = p
  a.offset = offset
  a.len = len
...
proc reset*(a: var Anim, len, speed: int, p: int = 0, offset: int = 0) =
  a.timer = speed + 1
  a.pos = p
  a.offset = offset
  a.len = len
...
proc animupdate*(a: var Anim) {.inline.} =
